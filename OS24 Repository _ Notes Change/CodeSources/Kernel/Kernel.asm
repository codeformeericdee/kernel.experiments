; Purpose: An assembly language kernel to run an operating system:
...
; This string is to inform the user/programmer which point in the boot process the
...
global _start               ; This defines a label to use as a public label.
_start:                     ; for the linker to know where to find each section.
                            ; it can be named anything, but the convention is to
                            ; name it _start.

[extern OperatingSystem]    ; Now that C language is being linked, its methods
                            ; can be added to the kernel by declaring labels
                            ; from the C source files as external
...
times 8192-($-$$) db 0  ; Pad the file to an arbitrary size for now.
                        ; I plan for making this a calculated size.